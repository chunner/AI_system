...
class MNIST_MLP(object):
    ...
    def load_mnist(self,file_dir, is_images = 'True'):
        ...
        if is_images:
           fmt_header = '>iiii'         # 代表读取四个大端（big-endian）整数：魔数（magic number）、图像数量、行数和列数。
           magic,num_images,num_rows,num_cols=struct.unpack_from(fmt_header,bin_data,0)
        else:
           fmt_header = '>ii'           # 代表两个大端整数：魔数和图像数量。
           magic,num_images=struct.unpack_from(fmt_header,bin_data,0)
           num_rows, num_cols= 1,1
        data_size = num_images * num_rows * num_cols        
        mat_data = struct.unpack_from('>' + str(data_size) + 'B', bin_data, struct.calcsize(fmt_header))        # 第一个参数表示读取 data_size 个无符号字节（B），格式为大端
                                                                                                                # 第二个参数表示计算文件头的大小，用于确定数据部分在文件中的起始位置。

        mat_data = np.reshape(mat_data,[num_images,num_rows*num_cols])              # 使用 np.reshape 将数据重塑为 [num_images, num_rows * num_cols] 的形状。
    ...
    def load_data(self):
        ...
        self.train_data=np.append(train_images,train_labels, axis=1)        # 将训练和测试数据与它们对应的标签合并
            ...
            for idx_batch in range(max_batch):
                ...
                if idx_batch % self.print_iter == 0:
                   ...
                   if(loss < self.lowest_loss):
                        ...
                        print('find lowest loss for now, saving model')
